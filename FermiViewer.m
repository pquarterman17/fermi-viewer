...
%   BosonPlotter, test_em_gui_harness
    ...
    % ── Tab 5: Export & Style ────────────────────────────────────────────
    tabExport = uitab(processTabGroup, 'Title', 'Export & Style');
    exportStyleGL = uigridlayout(tabExport, [6 2], ...
        'RowHeight', tabDef.RowHeight, 'ColumnWidth', tabDef.ColumnWidth, ...
        'Padding', tabDef.Padding, 'RowSpacing', tabDef.RowSpacing, ...
        'ColumnSpacing', tabDef.ColumnSpacing);

    btnFigureBuilder = uibutton(exportStyleGL, 'Text', 'Figure Builder...', ...
        'ButtonPushedFcn', @onFigureBuilder, ...
        'BackgroundColor', BTN_EXPORT, 'FontColor', BTN_FG, 'Enable', 'off', ...
        'Tooltip', 'Build multi-panel publication figure with labels, scale bars, and uniform sizing');
    btnFigureBuilder.Layout.Row = 1; btnFigureBuilder.Layout.Column = 1;

    btnJournalExport = uibutton(exportStyleGL, 'Text', 'Journal Export...', ...
        'ButtonPushedFcn', @onJournalExport, ...
        'BackgroundColor', BTN_EXPORT, 'FontColor', BTN_FG, 'Enable', 'off', ...
        'Tooltip', 'Export with journal-specific presets (Nature, Science, ACS, Elsevier)');
    btnJournalExport.Layout.Row = 1; btnJournalExport.Layout.Column = 2;

    btnPubPresets = uibutton(exportStyleGL, 'Text', 'Pub Presets', ...
        'ButtonPushedFcn', @onPubPresets, ...
        'BackgroundColor', BTN_EXPORT, 'FontColor', BTN_FG, 'Enable', 'off', ...
        'Tooltip', 'Apply journal-specific annotation formatting (APS/Nature/ACS)');
    btnPubPresets.Layout.Row = 2; btnPubPresets.Layout.Column = 1;

    btnCalibColorbar = uibutton(exportStyleGL, 'Text', 'Calib. Colorbar', ...
        'ButtonPushedFcn', @onCalibratedColorbar, ...
        'BackgroundColor', BTN_TOOL, 'FontColor', BTN_FG, 'Enable', 'off', ...
        'Tooltip', 'Add a calibrated colorbar with real-unit labels burned into exports');
    btnCalibColorbar.Layout.Row = 2; btnCalibColorbar.Layout.Column = 2;

    btnCustomCmap = uibutton(exportStyleGL, 'Text', 'Custom Colormap...', ...
        'ButtonPushedFcn', @onCustomColormap, ...
        'BackgroundColor', BTN_TOOL, 'FontColor', BTN_FG, 'Enable', 'off', ...
        'Tooltip', 'Create a custom colormap from user-defined color stops');
    btnCustomCmap.Layout.Row = 3; btnCustomCmap.Layout.Column = 1;

    btnColormapPreset = uibutton(exportStyleGL, 'Text', 'EM Colormaps', ...
        'ButtonPushedFcn', @onColormapPreset, ...
        'BackgroundColor', BTN_TOOL, 'FontColor', BTN_FG, 'Enable', 'off', ...
        'Tooltip', 'Auto-select colormap based on EM mode (SEM/TEM/STEM/EDS/phase)');
    btnColormapPreset.Layout.Row = 3; btnColormapPreset.Layout.Column = 2;

    btnColorOverlay = uibutton(exportStyleGL, 'Text', 'Overlay...', ...
        'ButtonPushedFcn', @onColorOverlay, ...
        'BackgroundColor', BTN_TOOL, 'FontColor', BTN_FG, 'Enable', 'off', ...
        'Tooltip', 'Blend two images with different colormaps in a new figure');
    btnColorOverlay.Layout.Row = 4; btnColorOverlay.Layout.Column = 1;

    btnBatchConvert = uibutton(exportStyleGL, 'Text', 'Batch Convert', ...
        'ButtonPushedFcn', @onBatchConvert, ...
        'BackgroundColor', BTN_EXPORT, 'FontColor', BTN_FG, 'Enable', 'off', ...
        'Tooltip', 'Convert loaded images between formats (PNG/TIFF/JPEG)');
    btnBatchConvert.Layout.Row = 4; btnBatchConvert.Layout.Column = 2;

    btnMacroRecord = uibutton(exportStyleGL, 'Text', 'Record Macro', ...
        'ButtonPushedFcn', @onMacroToggle, ...
        'BackgroundColor', BTN_TOOL, 'FontColor', BTN_FG, 'Enable', 'off', ...
        'Tooltip', 'Record measurement operations as a replayable macro');
    btnMacroRecord.Layout.Row = 5; btnMacroRecord.Layout.Column = 1;

    btnImgMath = uibutton(exportStyleGL, 'Text', 'Img Math...', ...
        'ButtonPushedFcn', @onImageMath, ...
        'BackgroundColor', BTN_TOOL, 'FontColor', BTN_FG, 'Enable', 'off', ...
        'Tooltip', 'Image arithmetic: subtract, divide, or ratio two loaded images');
    btnImgMath.Layout.Row = 5; btnImgMath.Layout.Column = 2;

    btnFlickerCompare = uibutton(exportStyleGL, 'Text', 'Flicker...', ...
        'ButtonPushedFcn', @onFlickerCompare, ...
        'BackgroundColor', BTN_TOOL, 'FontColor', BTN_FG, 'Enable', 'off', ...
        'Tooltip', 'Rapidly alternate between two images to spot subtle differences');
    btnFlickerCompare.Layout.Row = 6; btnFlickerCompare.Layout.Column = [1 2];

    % Per-tab grids exposed for theming (see setTheme)
    processTabGrids = {transformGL, filterGL, analysisGL, surfaceGL, exportStyleGL};
