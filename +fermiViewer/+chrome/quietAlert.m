...
%   so callers swap "uialert" → "fermiViewer.chrome.quietAlert" without changing
%   call sites otherwise.  When FERMI_VIEWER_HEADLESS=1 (see
%   fermiViewer.chrome.isHeadless), the call is suppressed and the message is
%   logged to the Command Window prefixed with [alert] so the test diary
%   still records what would have been shown.
%
%   Usage:
%       fermiViewer.chrome.quietAlert(fig, 'message');
%       fermiViewer.chrome.quietAlert(fig, 'message', 'Title');
%       fermiViewer.chrome.quietAlert(fig, 'message', 'Title', 'Icon', 'error');
