...
%
% Syntax
%   btn = fermiViewer.chrome.sectionHeader(parent, label, callback)
%   btn = fermiViewer.chrome.sectionHeader(__, name, value, ...)
%
% Inputs
...
%   Layout.Column, and update Text on subsequent toggles.
    ...
    if isempty(opts.Theme)
        tk = fermiViewer.chrome.uxTokens();
    else
        tk = fermiViewer.chrome.uxTokens(opts.Theme);
    end
