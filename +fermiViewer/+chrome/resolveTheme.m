...
%
% Syntax
%   t = fermiViewer.chrome.resolveTheme()            % reads themePref, resolves Auto
%   t = fermiViewer.chrome.resolveTheme('Auto')      % resolves Auto → Dark/Light
%   t = fermiViewer.chrome.resolveTheme('Dark')      % passthrough
%
...
% function returns 'Dark' rather than throwing.
    ...
    if nargin < 1 || isempty(value)
        value = fermiViewer.chrome.themePref('read');
    end
    value = char(value);
