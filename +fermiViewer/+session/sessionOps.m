...
%
%   Syntax
%     appData = fermiViewer.session.sessionOps('load',             appData, ctx)
%     appData = fermiViewer.session.sessionOps('renameBatch',      appData, ctx)
%     appData = fermiViewer.session.sessionOps('refreshImageList', appData, ctx)
%     appData = fermiViewer.session.sessionOps('fileDrop',         appData, ctx)
%
%   ctx fields (widgets)
...
switch action
    ...
    %SESSIONLOADAPI  Load a session .mat file and restore state.
        ...
        try
            ...
            if ~isfield(tmp, 'session')
                fermiViewer.chrome.quietAlert(fig, 'Not a valid session file.', 'Error', 'Icon', 'error');
            ...
            fermiViewer.chrome.quietAlert(fig, sprintf('Load failed:\n%s', ME.message), ...
    ...
    %RENAMEBATCH  Rename files on disk with baseName_001, _002, ... pattern.
        ...
        msg = sprintf('Rename %d file(s) on disk to %s_001, _002, ...?\nThis cannot be undone.', ...
            numel(idxs), baseName);
        answer = fermiViewer.chrome.quietConfirm(fig, msg, 'Confirm Batch Rename', ...
    ...
    % ────────────────────────────────────────────────────────────────────
    otherwise
        error('fermiViewer:session:sessionOps:unknownAction', ...
