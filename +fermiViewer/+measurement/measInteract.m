...
%
% Syntax:
%   appData = fermiViewer.measurement.measInteract('startEndpointDrag', appData, ctx, measIdx, whichEnd)
%   appData = fermiViewer.measurement.measInteract('applyMarqueeSelection', appData, ctx, xMin,xMax,yMin,yMax)
...
% Examples:
%   ctx = buildMeasCtx();
%   appData = fermiViewer.measurement.measInteract('startEndpointDrag', appData, ctx, midx, 1);
...
function appData = doOnAngleAction(appData, ctx, action)
    ...
    elseif nClicks >= 3
        ...
        angleDeg = fermiViewer.measurement.measurements('computeAngle', v1, v2, tiltDeg, tiltAxis, tiltGeom);
...
function appData = doOnPolylineAction(appData, ctx, action)
    ...
    if isDoubleClick && size(appData.captureClicks, 1) >= 2
        ...
        pts = appData.captureClicks;
        [tiltDeg, tiltAxis, tiltActive, tiltGeom] = ctx.cb.getTiltState();
        totalDist = fermiViewer.measurement.measurements('polylineLength', pts, tiltDeg, tiltAxis, tiltGeom);
