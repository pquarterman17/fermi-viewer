...
%   [appData, hMark] = fermiViewer.measurement.measExecute('endpointMarker', appData, ctx, x,y,symType,symColor)
%   [appData, hTxt]  = fermiViewer.measurement.measExecute('distLabel',      appData, ctx, x1,y1,x2,y2)
%   appData = fermiViewer.measurement.measExecute('runProfile', appData, ctx, x1,y1,x2,y2)
%   [appData, angleDeg] = fermiViewer.measurement.measExecute('angleFromPoints', appData, ctx, pts)
%   [appData, totalDist] = fermiViewer.measurement.measExecute('polylineFromPoints', appData, ctx, pts)
%
% Inputs:
...
switch lower(strtrim(action))
    ...
    otherwise
        error('fermiViewer:measurement:measExecute:unknownAction', ...
...
function appData = doRecomputeBox(appData, ctx, measIdx)
    ...
    try
        ...
        fermiViewer.chrome.quietAlert(ctx.fig, sprintf('Box profile failed:\n%s', ME.message), ...
            'Error', 'Icon', 'error');
        return;
    end
...
function appData = doRunProfile(appData, ctx, x1, y1, x2, y2)
    if isempty(appData.filteredPixels) || ~isnumeric(appData.filteredPixels)
        fermiViewer.chrome.quietAlert(ctx.fig, 'Load an image before running a line profile.', ...
    ...
    try
        ...
        fermiViewer.chrome.quietAlert(ctx.fig, sprintf('Line profile failed:\n%s', ME.message), ...
            'Error', 'Icon', 'error');
        return;
    end
...
%   pts is a 3x2 matrix: [vertex; ray1; ray2].
    ...
    angleDeg = fermiViewer.measurement.measurements('computeAngle', v1, v2, 0, 'Y', 'CrossSection');
    ...
    % Arc annotation
    arc = fermiViewer.measurement.measurements('arcGeometry', pts, v1, v2);
...
%   pts is an Nx2 matrix of (x,y) vertices (N >= 2).
    ...
    totalDist = fermiViewer.measurement.measurements('polylineLength', pts, 0, 'Y', 'CrossSection');
