...
%
% Syntax:
%   [appData, outHandles] = fermiViewer.interaction.captureDispatch(action, appData, ctx)
%   [appData, outHandles] = fermiViewer.interaction.captureDispatch(action, appData, ctx, arg1, ...)
%
% Actions:
...
% Examples:
%   ctx = buildCaptureCtx();
%   appData = fermiViewer.interaction.captureDispatch('rectClick', appData, ctx);
%   appData = fermiViewer.interaction.captureDispatch('startCapture', appData, ctx, 'distance');
%   appData = fermiViewer.interaction.captureDispatch('rectROI', appData, ctx, xMin, xMax, yMin, yMax);
    ...
    switch action
        ...
        otherwise
            error('fermiViewer:interaction:captureDispatch:unknownAction', ...
...
function appData = doExecuteRectROI(xMin, xMax, yMin, yMax, appData, ctx)
    ...
    r = fermiViewer.interaction.rectROI(appData.filteredPixels, xMin, xMax, yMin, yMax, imgInfoROI);
...
function appData = doBoxZoomDrag(appData, ctx)
    if strcmp(appData.dragAction, 'pan')
        ...
        [newXLim, newYLim] = fermiViewer.interaction.computePanLimits( ...
