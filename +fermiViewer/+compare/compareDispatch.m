...
%
% Syntax:
%   [appData, outHandles] = fermiViewer.compare.compareDispatch(action, appData, ctx)
%
% Actions:
...
%
%   [appData, ~] = fermiViewer.compare.compareDispatch('updateHighlight', appData, ctx);
    ...
    switch action
        ...
        otherwise
            error('fermiViewer:compare:compareDispatch:unknownAction', ...
...
function appData = doFlicker(appData, ctx)
    if ~isfield(appData, 'images') || numel(appData.images) < 2
        fermiViewer.chrome.quietAlert(ctx.fig, 'Load at least 2 images.', 'Need 2+ images');
        return;
    end
