...
%   result = fermiViewer.processing.executeFilter(pixels, 'gaussian', struct('sigma', 1.5))
%   result = fermiViewer.processing.executeFilter(pixels, 'median', struct('windowSize', 3))
%   result = fermiViewer.processing.executeFilter(pixels, 'clahe', struct('tileSize', 64, 'clipLimit', 3))
%
%   Returns struct with .pixels (filtered), .statusMsg, .applied (logical).
    ...
    switch lower(filterType)
        ...
        case 'clahe'
            result.pixels = fermiViewer.processing.applyCLAHE( ...
        ...
        otherwise
            error('fermiViewer:processing:unknownFilter', ...
