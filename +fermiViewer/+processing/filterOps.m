...
%   Syntax
%   ------
%   appData = fermiViewer.processing.filterOps(action, appData, fig, cb)
%   appData = fermiViewer.processing.filterOps(action, appData, fig, cb, ...)
%
%   Actions
%   -------
...
%   Examples
%   --------
%   appData = fermiViewer.processing.filterOps('gaussian', appData, fig, cb);
%   appData = fermiViewer.processing.filterOps('fftMaskAPI', appData, fig, cb, masks);
    ...
    switch action
        ...
        case 'gaussian'
            ...
            if isnan(sigma) || sigma <= 0
                fermiViewer.chrome.quietAlert(fig, 'Sigma must be a positive number.', 'Invalid Input', 'Icon', 'error');
                return;
            end
            ...
            try
                ...
                fermiViewer.chrome.quietAlert(fig, sprintf('Gaussian filter failed:\n%s', ME.message), ...
        ...
        case 'median'
            ...
            if isnan(wSize) || ~ismember(wSize, [3 5 7])
                fermiViewer.chrome.quietAlert(fig, 'Window size must be 3, 5, or 7.', 'Invalid Input', 'Icon', 'error');
                return;
            end
            fig.Pointer = 'watch'; drawnow;
            try
                ...
                fermiViewer.chrome.quietAlert(fig, sprintf('Median filter failed:\n%s', ME.message), ...
                    'Filter Error', 'Icon', 'error');
            end
            fig.Pointer = 'arrow';
        ...
        case 'liveFFTToggle'
            ...
            if src.Value
                ...
                appData = fermiViewer.processing.filterOps('updateLiveFFT', appData, fig, cb);
            else
        ...
        otherwise
            warning('fermiViewer:processing:filterOps:unknownAction', ...
