...
%
% Syntax
%   appData = fermiViewer.processing.processActions(action, appData, ctx)
%
% Inputs
...
switch action
    ...
    case 'roughness'
        ...
        try
            ...
            fermiViewer.chrome.quietAlert(ctx.fig, fermiViewer.display.formatRoughnessResult(result, pu), ...
    ...
    case 'interfaceFit'
        ...
        if ~isfield(appData, 'lastProfile') || isempty(appData.lastProfile)
            fermiViewer.chrome.quietAlert(ctx.fig, 'Draw a line profile first, then click Interface Fit.', 'No profile');
            return;
        end
        try
            ...
            fermiViewer.chrome.quietAlert(ctx.fig, msg, 'Interface Fit', 'Icon', 'info');
    ...
    case 'multiOtsu'
        ...
        if isnan(nClass) || nClass < 2 || nClass > 5
            fermiViewer.chrome.quietAlert(ctx.fig, 'Classes must be 2-5.', 'Invalid'); return;
        end
        try
    ...
    case 'gpa'
        ...
        px = ctx.guiPixelSize();
        if px <= 0
            fermiViewer.chrome.quietAlert(ctx.fig, 'Set pixel calibration first for meaningful strain values.', 'No calibration');
        end
    ...
    case 'ctfEstimate'
        ...
        if any(isnan([kV, Cs, pxA]))
            fermiViewer.chrome.quietAlert(ctx.fig, 'Invalid numeric input.', 'Error'); return;
        end
        try
    ...
    case 'backProject'
        ...
        if ~isfield(appData, 'images') || numel(appData.images) < 2
            fermiViewer.chrome.quietAlert(ctx.fig, 'Load a tilt series (multi-frame) first.', 'Need stack'); return;
        end
    ...
    case 'figureBuilder'
        ...
        if nImg < 1
            fermiViewer.chrome.quietAlert(ctx.fig, 'Load at least one image.', 'No images'); return;
        end
