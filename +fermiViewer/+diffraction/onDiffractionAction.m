...
%
% Syntax:
%   appData = fermiViewer.diffraction.onDiffractionAction(action, appData, ui, callbacks)
%
% Inputs:
...
%
% Examples:
%   appData = fermiViewer.diffraction.onDiffractionAction('clearSpots', appData, ui, cb);
%   appData = fermiViewer.diffraction.onDiffractionAction('match', appData, ui, cb);
...
switch action
    ...
    case 'latticeMeasure'
        ...
        px = guiPixelSize();
        if px <= 0
            fermiViewer.chrome.quietAlert(fig, 'Set pixel calibration first (pixel size > 0).', 'No calibration');
            return;
        end
    ...
    case 'latticeExecute'
        ...
        try
            ...
            fermiViewer.chrome.quietAlert(fig, msg, 'Lattice Measurement', 'Icon', 'info');
