%TEST_HISTOGRAM_OVERLAY  Smoke test for fermiViewer.contrast.drawHistogramOverlay.
...
try
    ...
    fermiViewer.contrast.drawHistogramOverlay(ax, 30, 70, 1.0, 'linear', false, rawPx);
...
try
    ...
    fermiViewer.contrast.drawHistogramOverlay(ax, 20, 80, 2.5, 'linear', false, rawPx);
...
try
    ...
    ax.XLim = [0 100];
    fermiViewer.contrast.drawHistogramOverlay(ax, 10, 90, 1.0, 'linear', false, rawPx);
...
try
    delete(findobj(ax, 'Tag', 'histMarker'));
    fermiViewer.contrast.drawHistogramOverlay(ax, 30, 70, 1.0, 'linear', false, rand(500,1)*100, ...
...
try
    for tf = {'log','sqrt','power','linear'}
        delete(findobj(ax, 'Tag', 'histMarker'));
        fermiViewer.contrast.drawHistogramOverlay(ax, 5, 95, 1.5, tf{1}, false, rand(500,1)*100);
...
try
    delete(findobj(ax, 'Tag', 'histMarker'));
    fermiViewer.contrast.drawHistogramOverlay(ax, 20, 80, 1.0, 'linear', true, rand(500,1)*100);
    rampLine = findobj(ax, 'Tag', 'histMarker', 'Type', 'line', 'LineStyle', '-');
